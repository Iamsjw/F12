...
class _AttendanceRowState extends State<_AttendanceRow>
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              if (widget.student.deviceId != null && widget.student.deviceId!.isNotEmpty) ...[
                IconButton(
                  icon: const Icon(
                  ...
                  onPressed: () => _resetDeviceLock(context),
  ...
  Future<void> _resetDeviceLock(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to reset the device lock for ${widget.student.name}? This will allow them to register a new phone on their next attendance marking.',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
            color: AppTheme.textSecondary,
          ),
    ...
    if (confirmed == true) {
      ...
      final success = await SupabaseService.resetStudentDevice(widget.student.id);
      if (success) {
        if (widget.onRefreshStudents != null) {
          widget.onRefreshStudents!();
        }
        messenger.showSnackBar(
          SnackBar(
            content: Text(
              'Device lock reset successfully for ${widget.student.name}.',
